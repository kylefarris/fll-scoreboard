...
export default class CommitForm implements m.ClassComponent<CommitFormAttrs> {
  ...
  view(vnode: m.Vnode<CommitFormAttrs, this>) {
    ...
    return m('div', [
      m('.form-lock', [
        ...
        m(
          ...
          [
            ...
            ` ${scorecard.commitForm.scoreLocked ? 'Unl' : 'L'}ock Scores`
          ]
        )
      ]),
      m('.team-signature-form', { style: `display: ${scorecard.commitForm.scoreLocked || scorecard.commitForm.scoreApproved ? 'block' : 'none'};` }, [
        m('h3', 'Team Zone'),
        m('p', 'Please verify the results. Once you are satisfied, type intials into the field below and press "Approve Score".'),
          ...
          [
            m('.field-group', [
              ...
              m('input', {
                ...
                class: 'input-field',
                maxlength: 3,
                readonly: scorecard.commitForm.scoreApproved,
                oninput(e) {
                  scorecard.commitForm.teamMemberInitials = e.target.value.replace(/[^A-Z]/gi, '').toUpperCase();
                },
                onblur(e) {
                  scorecard.commitForm.teamMemberInitials = e.target.value.toUpperCase();
                },
                value: scorecard.commitForm.teamMemberInitials,
              }),
            ]),
            m(
              'button',
              {
                type: 'submit',
                disabled: scorecard.commitForm.teamMemberInitials.trim().length < 2 || scorecard.commitForm.scoreApproved
              },
              'Approve Score'
            )
          ]
      ...
      m('.gameday-form', { style: `display: ${scorecard.commitForm.scoreApproved ? 'block' : 'none'};` },
      [
        m('h3', 'Referee Zone'),
        m('p', 'Once the team has verified the score with you, please provide the necessary information to officially submit this score to the FLL Gameday system.'),
        m(
              ...
              if (scorecard.checkFormSubmitable()) {
                try {
                  ...
                  if (result === true) {
                    ...
                    M.toast({
                      html: scorecard.noWifi ? 'Score Saved Locally Only' : 'Score Received!',
          ...
          [
            m('.field-group', [
              scorecard.refError ? m('small.error-info', scorecard.refError) : undefined,
              m('label', 'Your Referee Code'),
              m('input', {
                type: 'text',
                name: 'refCode',
                maxlength: 6,
                disabled: !scorecard.commitForm.scoreApproved,
                class: `input-field${scorecard.refError !== null ? ' invalid' : ''}`,
                async onblur(e) {
            ...
            m(
              ...
              {
                type: 'submit',
                disabled: (!scorecard.commitForm.teamMemberInitials || !scorecard.commitForm.refCode || scorecard.committing) && !scorecard.noWifi
              },
              'Submit Score'
            )
          ]
